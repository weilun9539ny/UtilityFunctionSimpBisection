...
for (i in dat.all$index) {
  par <- c(
    dat.all$alpha[i], dat.all$beta[i],
    dat.all$alpha.bayes[i], dat.all$beta.bayes[i]
  )
  label <- c(
    paste("(F) alpha:", round(par[1], 3)),
    paste("(F) beta:", round(par[2], 3)),
    paste("(B) alpha:", round(par[3], 3)),
    paste("(B) beta:", round(par[4], 3))
  )
  subj.plot <- dat.long %>% 
    filter(subj_id == i) %>% 
    ggplot(aes(x=x, y=y)) +
