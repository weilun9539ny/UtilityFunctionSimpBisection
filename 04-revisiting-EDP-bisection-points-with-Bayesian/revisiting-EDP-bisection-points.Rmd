...
res.df <- dat %>% 
  transmute(
    ...
    alpha.bayes.mean = value_func.summary[6:92, 2][[1]],
    beta.bayes.mean = value_func.summary[93:179, 2][[1]],
    alpha.bayes.median = value_func.summary[6:92, 3][[1]],
    beta.bayes.median = value_func.summary[93:179, 3][[1]],
    alpha.nls = NA,
    beta.nls = NA
  )
...
# res.df %>% write.csv("./result/estimation-nls-HB.csv", row.names = F)
res.df %>% head(10)
```
...
for (i in res.df$subj_id) {
  ...
  (subj.plot <- dat.long %>% 
    ...
    theme_light())
  # print(subj.plot)
  save.plot(4, c("subject_plot", res.df$subject[i]))
}
