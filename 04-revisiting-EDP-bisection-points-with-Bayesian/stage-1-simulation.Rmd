...
```{r test-experiment}
experiment(c(0.576049517, 1.249581918, 1, .0367))
```
...
# pars.df %>% write.csv("../01-data/generated_data/subject_alpha-beta-phi.csv", row.names=F)
# pars.df <- read.csv("../01-data/generated_data/subject_alpha-beta-phi.csv")
pars.df %>% head()
```
...
for (subj in pars.df$subj_id) {
  pars <- pars.df[subj, 2:5] %>% 
    unlist %>% 
    unname
  res <- experiment(pars)
  
  # store the result
  dat.exp <- rbind(dat.exp, res$points)
  file.name <- paste0( 
    "subj-", subj, 
    "_alpha-", res$pars[[1]], 
    "_beta-", res$pars[[2]], 
...
# dat.exp %>% write.csv("../01-data/generated_data/simulation-result.csv", row.names=F)
dat.exp %>% str()
```


## 5. Non-Linear Least Square Estimation

開始估計前，先整理資料。

```{r data-arrange}
dat.long <- dat.exp %>% 
  mutate(subj_id = row_number(), .before=1) %>% 
...
est.res <- pars.df %>% 
  mutate(
    alpha.index = paste0("alpha[", subj_id, "]"),
    beta.index = paste0("beta[", subj_id, "]"),
    alpha.bayes.mean = NA,
    beta.bayes.mean = NA,
    alpha.bayes.median = NA,
    beta.bayes.median = NA,
    alpha.nls = NA,
    beta.nls = NA
  )

for (j in dat.exp$index) {
  # fit non-linear least square
  dat.pos.cur <- dat.pos %>% 
    filter(subj_id == j)
  dat.neg.cur <- dat.neg %>% 
    filter(subj_id == j)
  res.alpha <- nls(y ~ x^alpha, dat.pos.cur, start=list(alpha=0)) %>% coef(F)
  res.beta <- nls(y ~ -x^beta, dat.neg.cur, start=list(beta=0)) %>% coef(F)
  
  # store the result
  res.df[j, 9:10] <- c(res.alpha, res.beta)
}

rm(dat.pos.cur, dat.neg.cur)
```
