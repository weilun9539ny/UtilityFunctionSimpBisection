...
To sum up, CPT has 6 parameters: $\alpha \in [0, 1]$, $\beta \in [0, 1]$, $\lambda \in (0, \infty)$, $\gamma \in [0, 1]$, $\delta \in [0, 1]$, and $\phi \in (0, \infty)$.
...
+ $\gamma$ quantifies the shape of probability weighting function for gains;
+ $\delta$ quantifies the shape of probability weighting function for losses;
+ $\phi$ quantifies the extent to which choice behavior is guided by subjective values.

### Parameter Setting

Parameters $\alpha_i, \beta_i, \gamma_i$ and $\delta_i$ were first transformed to probit scale: 
$\alpha_i^\phi = \Theta^{-1}(\alpha_i)$, $\beta_i^\phi = \Theta^{-1}(\beta_i)$, $\gamma_i^\phi = \Theta^{-1}(\gamma_i)$ and $\delta_i^\phi = \Theta^{-1}(\delta_i)$.
Thus, $\alpha^\phi, \beta^\phi, \gamma^\phi, \delta^\phi \in \mathbb{R}$.

Next, we assumed these subject-level parameters come from group-level normal distributions: 
$\alpha^\phi_i \sim N(\mu^\alpha, \sigma^\alpha)$, $\beta^\phi_i \sim N(\mu^\beta, \sigma^\beta)$, $\gamma^\phi_i \sim N(\mu^\gamma, \sigma^\gamma)$, and $\delta^\phi_i \sim N(\mu^\delta, \sigma^\delta)$.

Finally, we decided the priors to the group-level parameters:
$\mu^\alpha \sim N(0, 1)$, $\mu^\beta \sim N(0, 1)$, $\mu^\gamma \sim N(0, 1)$, and $\mu^\delta \sim N(0, 1)$.
$\sigma^\alpha \sim U(0, 10)$, $\sigma^\beta \sim U(0, 10)$, $\sigma^\gamma \sim U(0, 10)$, and $\sigma^\delta \sim U(0, 10)$.
...
Since we want $\lambda, \phi \in [0.1, 5]$, the hyperparameters are $\mu^\lambda\sim U(-2.3, 1.61)$, $\mu^\delta\sim U(-2.3, 1.61)$, $\sigma^\lambda\sim U(0, 1.13)$, and $\sigma^\delta\sim U(0, 1.13)$.
...
```{r MCMC plots}
params <- c(
  paste0("alpha[", 1:4, "]"), paste0("beta[", 1:4, "]"), 
  paste0("gamma[", 1:4, "]"), paste0("delta[", 1:4, "]"),
  paste0("lambda[", 1:4, "]"), paste0("phi[", 1:4, "]")
)
mcmc_trace(fit, pars = params[17:24]) +
  labs(title = "Trace plot")

mcmc_dens_overlay(fit, pars = params[13:24]) +
