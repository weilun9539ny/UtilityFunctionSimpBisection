...
Thus, we took out every 5th trials.
As for those who did not response to these trials, we took the second last decision.
...
# saveRDS(fit_full, file="./res/CPT_full_c-4_ite-5e3_warm-1e3.rds")
# fit_full <- readRDS(file="./res/CPT_full_c-4_ite-5e3_warm-1e3.rds")
# summary(fit_full)
```
...
```{r CPT alpha=beta}
fit_no.beta <- stan(
  file = "./CPT_model/hierarchical_CPT_no-beta.stan",
  data = stan_data,
  chains = 4,
  iter = 5e3,
  warmup = 1000,
...
# saveRDS(fit_alpha.lambda, file="./res/CPT_no-beta_c-4_ite-5e3_warm-1e3.rds")
# fit_no.beta <- readRDS(file="./res/CPT_no-beta_c-4_ite-5e3_warm-1e3.rds")
```
...
# saveRDS(fit_no.lambda, file="./res/CPT_no-lambda_c-4_ite-5e3_warm-1e3.rds")
# fit_no.lambda <- readRDS(file="./res/CPT_no-lambda_c-4_ite-5e3_warm-1e3.rds")
```
...
params.group_alpha.lambda <- paste("mu", c("alpha", "loglambda", "logphi"), sep="_")
